...
void UTMConverter::latitudeAndLongitudeToUTMCoordinates(double lat, double lng, UTMCoordinates &utm)
{
    unsigned int zone = floor((lng + 180.0) / 6) + 1;
...
double UTMConverter::UTMCentralMeridian(unsigned int zone)
{
    ...
    c_meridian = DEG2RAD(-183.0 + (zone * 6.0));
